    ...
    <form v-on:submit.prevent="filter"
        ...
        <div id="seachFilter" class="row">
          <div class="gr-12">
            ...
            <div class="gr-3">
              <div class="form-group padding-5">
                ...
                <multiselect style="z-index:100" v-model="accountTypeChoice" :options="accountTypeList" :multiple="true" :close-on-select="false" :clear-on-select="false" :hide-selected="true" placeholder="Select account types ..." label="name" track-by="value">
                </multiselect>
              </div>
            </div>
            ...
            <div class="gr-3">
              <div class="form-group padding-5">
                ...
                <multiselect style="z-index:100" v-model="verificationStatusChoice" :options="verificationStatusList" :multiple="true" :close-on-select="false" :clear-on-select="false" :hide-selected="true" placeholder="Select verification status ..." label="name" track-by="value">
                </multiselect>
              </div>
            </div>
          </div>
          ...
          <!-- <div class="gr-12">
            ...
            <div v-if="members.list && members.list.length > 0" class="gr-1 push-9">
              <div class="select select-sm">
                <select v-model="query.pageSize" @change="filter">
                  <option disabled>Number of Entries</option>
                  <option selected value="10">10</option>
  ...
  export default {
    ...
    data () {
      return {
        ...
        genderChoice: [],
        idTypeList: [
          {value: 'national_id', name: 'NID'},
          {value: 'passport', name: 'Passport'},
          {value: 'driving_license', name: 'Driving License'},
          {value: 'birth_certificate', name: 'Birth Certificate'},
          {value: 'tin', name: 'TIN'},
          {value: 'other', name: 'Other'},
          {value: 'business_tin', name: 'Business TIN'},
          {value: 'trade_license', name: 'Trade License'},
          {value: 'vat_reg_certificate', name: 'VAT Registration Certificate'},
          {value: 'merchant_acquisition_form', name: 'Merchant Aquisition Form'},
          {value: 'consent_agreement', name: 'Consent Agreement'}
        ],
        idTypeChoice: [],
        bankDocList: [
    ...
    methods: {
      ...
      getMembers: function (key = 'memberSearch') {
        this.checkBoxTicked = []
        for (let i = 0; i < 100; i++) {
...
//        })
      ...
      init () {
        ...
        this.verificationStatusChoice = []
        this.accountTypeChoice = []
        this.genderChoice = []
        this.idTypeChoice = []
        this.bankDocChoice = []
      ...
      filter (key = 'memberSearch') {
        this.query.verificationStatus = this.verificationStatusChoice.map(x => x.value)
        this.query.accountType = this.accountTypeChoice.map(x => x.value)
        ...
        this.showLoader = true
        Http.GET('memberSearch', this.query)
          .then(({data: {data}}) => {
            console.log('sortParam: ', this.query.sort, 'orderParam: ', this.query.order)
            if (this.query.sort === 'PROFILE_COMPLETION_SCORE') {
              this.sortParam = 'PROFILE COMPLETION SCORE'
